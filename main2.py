...
# ---------- OUTPUT DIRECTORIES ----------
OUTPUT_ROOT = Path("/Users/mariamlocka/Documents/project_AI_interfaces_final/data")
...
def style_extraction(root_path: Path) -> pd.DataFrame:
    """
    Detect the first .css file under root_path and extract styles.
    """
    css_file = next(root_path.rglob("*.css"), None)
    project_name = get_project_name(css_file.relative_to(root_path))

    if not css_file:
        print(f"⚠️ No .css file found under {root_path}")
        return pd.DataFrame()

    df_styles = extract_css_variables_and_fonts(css_file)
    df_styles["project_name"] = project_name
    output_style_file = STYLE_OUTPUT_DIR / f"extracted_styles.csv"
